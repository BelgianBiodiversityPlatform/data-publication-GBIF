...
```{r}
occurrence %<>% mutate(datasetID = "https://doi.org/complete_this")
```
...
```{r}
occurrence %<>% mutate(rightsHolder = institutionCode) # e.g. "INBO"
```
### accessRights
...
```
```{r}
occurrence %<>% mutate(verbatimDatasetName = datasetName) 
```
...
```{r}
occurrence %<>% mutate(datasetName = "A database of common vampire bat reports") # e.g. "Checklist of non-native freshwater fishes in Flanders, Belgium"
```
...
```{r}
occurrence %<>% mutate(basisOfRecord = replace_na(basisOfRecord,'HumanObservation'))
                
```
```{r}
##Occurrence %<>% mutate(basisOfRecord = replace(basisOfRecord, 'UNKNOWN', 'HumanObservation'))
```
...
```{r}
occurrence %<>% mutate(institutionCode = replace_na(institutionCode,'unk'))
```
```{r}
occurrence %<>% mutate(institutionID = replace_na(institutionID, 'unk'))
```


```{r}
occurrence %<>% mutate(occurrenceID = institutionID) %<>%
                mutate(occurrenceID = str_c(ID_row, ":", institutionCode, ":" , occurrenceID))
